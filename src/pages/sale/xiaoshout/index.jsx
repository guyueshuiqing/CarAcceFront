...
import { getAllXiaoShouT, getOwnXiaoShouT, getFilterXiaoShouT, updateXiaoShouTDate,updateXiaoShouTPrice } from '../../../services/servers'
import { tSelect, dataSource, getTCols } from '../config'

@connect(state => ({
  cms: state.cms
}))

class XiaoShouT  extends Component {
  constructor(props) {
    super(props);
    this.state = { 
      tableData: [],
  ...
  submitForm = (values) => {
    ...
    getFilterXiaoShouT(params).then(res=>{
      if(res && !res.flag){
        message.error(res.message && res.message)
        return
      }
  ...
  newFormSubmit = (values) =>{
    ...
    if(actionModal === 'updateDate' || actionModal === 'verifyDate'){
      ...
      updateXiaoShouTDate(params).then(res=>{
        if(res && !res.flag){
          message.error(res.message && res.message)
          return
        }
    ...
    if(actionModal === 'applyPrice'){
      ...
      updateXiaoShouTPrice(params).then(res=>{
        if(res && !res.flag){
          message.error(res.message && res.message)
          return
        }
